    ...
    def asyncGet(self):
        ...
        try:
            ...
            dbData = glob.db.fetch(
                "SELECT beatmap_md5, beatmapset_id FROM beatmaps WHERE beatmap_id = {}".format(
                    beatmapID
                )
            )
            if dbData is None:
            ...
            if accuracy < 0 and modsEnum == 0:
                ...
                log.debug(
                    "Specific request ({}%/{}). Calculating pp with peace...".format(
                        accuracy, modsEnum
                    )
                )
                peace = ez_peace.EzPeace(bmap, mods_=modsEnum, tillerino=True)
            ...
            data = {
                ...
                "pp": [round(x, 2) for x in returnPP]
                if type(returnPP) == list
                else round(returnPP, 2),
