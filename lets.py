    ...
    return tornado.web.Application(
        [
            ...
            (
                ...
                dict(destination="https://kurikku.pw/"),
            ),
            (
                r"/u/(.*)",
                redirectHandler.handler,
                dict(destination="https://kurikku.pw/u/{}"),
            ),
            (
                r"/b/(.*)",
                redirectHandler.handler,
                dict(destination="https://kurikku.pw/b/{}"),
            ),
            (r"/", redirectHandler.handler, dict(destination="https://kurikku.pw/")),
            (
                r"/beatmapsets/(.*)",
                redirectHandler.handler,
                dict(destination="https://kurikku.pw/beatmapsets/{}"),
            ),
...
if __name__ == "__main__":
    try:
        ...
        if not glob.conf.checkConfig():
            ...
            consoleHelper.printColored(
                "[!] Invalid config.ini. Please configure it properly", bcolors.RED
            )
            consoleHelper.printColored(
                "[!] Delete your config.ini to generate a default one", bcolors.RED
            )
            sys.exit()
        else:
        ...
        ranked_mods = [
            item
            for item in glob.conf.extra["rankable-mods"]
            if glob.conf.extra["rankable-mods"][item]
        ]
        unranked_mods = [
            item
            for item in glob.conf.extra["rankable-mods"]
            if not glob.conf.extra["rankable-mods"][item]
        ]
        consoleHelper.printColored(
            "Ranked mods  : {}".format(", ".join(ranked_mods)), bcolors.YELLOW
        )
        consoleHelper.printColored(
            "Unranked mods: {}".format(", ".join(unranked_mods)), bcolors.YELLOW
        )
        glob.conf.extra["_unranked-mods"] = sum(
            [getattr(mods, item) for item in unranked_mods]
        )  # Store the unranked mods mask in glob
        ...
        try:
            ...
            if glob.sentry:
                glob.application.sentry_client = AsyncSentryClient(
                    glob.conf.config["sentry"]["dns"], release=glob.VERSION
                )
            else:
                consoleHelper.printColored(
        ...
        try:
            if generalUtils.stringToBool(glob.conf.config["datadog"]["enable"]):
                ...
                consoleHelper.printColored(
                    "[!] Warning! Datadog stats tracking is disabled!", bcolors.YELLOW
                )
        except:
            consoleHelper.printColored(
        ...
        consoleHelper.printColored(
            "> L.E.T.S. is listening for clients on 127.0.0.1:{}...".format(serverPort),
            bcolors.GREEN,
        )
